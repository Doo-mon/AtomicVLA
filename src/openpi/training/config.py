...
class LeRobotCalvinDataConfig(DataConfigFactory):
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        data_transforms = _transforms.Group(
            inputs=[calvin_policy.CalvinInputs(action_dim=model_config.action_dim, model_type=model_config.model_type)],
...
class TrainConfig:
    ...
    # -- see BaseModelConfig. Specific model implementations (e.g., Pi0AtomicConfig) inherit from BaseModelConfig and may
    # define additional attributes.
    model: _model.BaseModelConfig = dataclasses.field(default_factory=pi0_config.Pi0AtomicConfig)
...
_CONFIGS = [
    ...
    TrainConfig(
        name="pi05_aloha",
        model=pi0_config.Pi0AtomicConfig(pi05=True),
        data=LeRobotAlohaDataConfig(
            assets=AssetsConfig(asset_id="trossen"),
        ),
        policy_metadata={"reset_pose": [0, -1.5, 1.5, 0, 0, 0]},
    ...
    TrainConfig(
        name="pi0_droid",
        model=pi0_config.Pi0AtomicConfig(action_horizon=10),
    ...
    TrainConfig(
        name="pi05_droid",
        model=pi0_config.Pi0AtomicConfig(action_horizon=15, pi05=True),
        data=SimpleDataConfig(
            assets=AssetsConfig(asset_id="droid"),
            data_transforms=lambda model: _transforms.Group(
    ...
    TrainConfig(
        name="pi05_calvin",
        model=pi0_config.Pi0AtomicConfig(pi05=True, action_horizon=15, discrete_state_input=False),
        data=LeRobotCalvinDataConfig(
            repo_id="your_hf_username/calvin_30hz",
    ...
    TrainConfig(
        ...
        data=FakeDataConfig(),
        batch_size=2,
        model=pi0_config.Pi0AtomicConfig(paligemma_variant="dummy", action_expert_variant="dummy"),
        save_interval=100,
        overwrite=True,
        exp_name="debug",
        num_train_steps=10,
    ...
    TrainConfig(
        name="debug_pi05",
        model=pi0_config.Pi0AtomicConfig(pi05=True, paligemma_variant="dummy", action_expert_variant="dummy"),
        data=FakeDataConfig(),
        batch_size=2,
        num_train_steps=10,
        overwrite=True,
