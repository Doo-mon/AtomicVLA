...
def preprocess_observation(
    ...
    observation: Observation | AtomicObservation,
    *,
    train: bool = False,
    ...
    if not isinstance(observation, AtomicObservation):
        return Observation(
            images=out_images,
            ...
            if isinstance(target, dict) and isinstance(source, dict):
                ...
                for k, v in target.items():
                    if k in source:
                        ...
                        # target 有但 source 没有 → 用 target 的值（通常是 None 或 ShapeDtypeStruct）
                        aligned[k] = v
                return aligned
            else:
                return source  # 叶子节点直接返回

        params_aligned = align_to_target(state.to_pure_dict(), params)
        at.check_pytree_equality(expected=state.to_pure_dict(), got=params_aligned, check_shapes=True, check_dtypes=False)
        state.replace_by_pure_dict(params_aligned)
