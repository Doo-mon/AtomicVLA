...
from multistep_sequences import get_sequences
from utils import get_env_state_for_initial_condition
import calvin_env
...
class Args:
    ...
    #################################################################################################################
    calvin_data_path: str = ""  # Path to CALVIN dataset for loading validation tasks
    ...
    num_save_videos: int = 100  # Number of videos to be logged per task
    video_temp_subsample: int = 3  # Temporal subsampling to make videos shorter

    seed: int = 42  # Random Seed (for reproducibility)

def get_category(task_name):
    verb = task_name.split('_')[0]
    if verb in {'open', 'close'}:
        return 'open_close'
    elif verb in {'stack', 'unstack'}:
        return 'stack'
    else:
        return verb
...
def main(args: Args) -> None:
    ...
    BAD_ACTION_TEMPLATE = np.array([[-0.01258917, -0.11098764,  0.07629602,  2.0664371,   0.04638874, -0.08491437,
    -0.14932624]] * 15)
            ...
            for _ in range(args.max_subtask_steps):
                ...
                if not action_plan:
                    ...
                    element = {
                        ...
                        # "thought":[str(task_instructions[subtask][0]), "the atomic skill is " + get_category(subtask)]  #for test code
                    }

                    
                    resut = client.infer(element)
                    action_chunk = resut["actions"]

                    while len(resut["thoughts"].split()) >= 2:
                            resut = client.infer(element)
                            action_chunk = resut["actions"]
...
def _get_calvin_tasks_and_reward(num_sequences):
    conf_dir = pathlib.Path(calvin_env.__file__).absolute().parents[2] / "calvin_env" / "conf"
    task_cfg = OmegaConf.load(conf_dir / "tasks/new_playtable_tasks.yaml")
    task_oracle = hydra.utils.instantiate(task_cfg)
    val_annotations = OmegaConf.load(conf_dir / "tasks/new_playtable_validation.yaml")
